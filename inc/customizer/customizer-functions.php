...
if (!function_exists('viral_express_widget_list')) {

    function viral_express_widget_list() {
        global $wp_registered_sidebars;
        $exclude = viral_express_get_default_widgets();
        $exclude = array_merge($exclude, array('viral-express-frontpage-right-sidebar', 'viral-express-frontpage-left-sidebar'));
        $widget_list['none'] = esc_html__('-- Choose Widget --', 'viral-express');
...
if (!function_exists('viral_express_cat')) {

    function viral_express_cat() {
        $cat = array();
...
if (!function_exists('viral_express_page_choice')) {

    function viral_express_page_choice() {
        $page_choice = array();
...
if (!function_exists('viral_express_menu_choice')) {

    function viral_express_menu_choice() {
        $menu_choice = array('none' => esc_html('-- Choose Menu --', 'viral-express'));
...
if (!class_exists('Viral_Express_Walker_Category_Checklist')) {

    class Viral_Express_Walker_Category_Checklist extends Walker {
...
if (!function_exists('viral_express_terms_checklist')) {

    function viral_express_terms_checklist($post_id = 0, $args = array(), $terms = NULL) {
        $defaults = array(
            'selected_cats' => false,
            'walker' => null,
            'taxonomy' => 'category',
        ...
        if (empty($walker) || !is_a($walker, 'Walker')) {
            $walker = new Viral_Express_Walker_Category_Checklist;
        }
...
if (!function_exists('viral_express_get_dropdown_indent')) {

    function viral_express_get_dropdown_indent($parent_id, $categories, $selected_ids = [], $cat_ids = [], $child_count = -1) {
        $html = '';
        ...
        if (count($loop_categories)) {
            ...
            if ($loop_categories) {
                foreach ($loop_categories as $cat) {
                    ...
                    $child_html = viral_express_get_dropdown_indent($cat->term_id, $categories, $selected_ids, $cat_ids, $child_count);
...
if (!function_exists('viral_express_icon_choices')) {

    function viral_express_icon_choices() {
        ...
        echo '</select>';
        echo '<input type="text" class="ht--icon-search-input" placeholder="' . esc_html__('Type to filter', 'viral-express') . '" />';
        echo '</div>';
...
add_action('customize_controls_print_footer_scripts', 'viral_express_icon_choices');

function viral_express_order_sections() {
    if (!current_user_can('manage_options')) {
        return;
    }

    check_ajax_referer('viral-express-order-sections', 'secure');

    if (isset($_POST['sections'])) {
        set_theme_mod('viral_express_frontpage_sections', $_POST['sections']);
    }
    wp_die();
}

add_action('wp_ajax_viral_express_order_sections', 'viral_express_order_sections');

function viral_express_frontpage_sections() {
    $defaults = array(
        'viral_express_frontpage_slider1_section',
        'viral_express_frontpage_mininews_section',
        'viral_express_frontpage_leftnews_section',
        'viral_express_frontpage_rightnews_section',
        'viral_express_frontpage_carousel1_section',
    );
    $sections = get_theme_mod('viral_express_frontpage_sections', $defaults);
    return $sections;
}

function viral_express_get_section_position($key) {
...
function viral_express_scroll_top_icons_array() {
...
function viral_express_check_cfu() {
