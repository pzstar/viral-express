...
if (!class_exists('Viral_Express_Walker_Widget_Category_Checklist')) {

    class Viral_Express_Walker_Widget_Category_Checklist extends Walker {
...
if (!function_exists('viral_express_widgets_terms_checklist')) {

    function viral_express_widgets_terms_checklist($post_id = 0, $args = array(), $terms = NULL) {
        $defaults = array(
            'selected_cats' => false,
            'walker' => null,
            'taxonomy' => 'category',
        ...
        if (empty($walker) || !is_a($walker, 'Walker')) {
            $walker = new Viral_Express_Walker_Widget_Category_Checklist;
        }
...
function viral_express_widgets_terms_list($parent_id, $categories, $cat_ids = [], $child_count = -1) {
    $options = array();
    ...
    if (count($loop_categories)) {
        ...
        if ($loop_categories) {
            foreach ($loop_categories as $cat) {
                $cat_name = '';
                $child_options = viral_express_widgets_terms_list($cat->term_id, $categories, $cat_ids, $child_count);
...
function viral_express_category_list() {
 ...
 */
function viral_express_admin_scripts() {
    ...
    if (is_rtl()) {
        wp_enqueue_style('viral-express-admin-style', get_template_directory_uri() . '/inc/widgets/css/widget-style.rtl.css', array('wp-color-picker'), VIRAL_EXPRESS_VER);
    } else {
        wp_enqueue_style('viral-express-admin-style', get_template_directory_uri() . '/inc/widgets/css/widget-style.css', array('wp-color-picker'), VIRAL_EXPRESS_VER);
    }

    wp_enqueue_media();
    $is_widgets_block_editor = function_exists('wp_use_widgets_block_editor') && wp_use_widgets_block_editor() ? 'true' : 'false';
    wp_enqueue_script('viral-express-widget-script', get_template_directory_uri() . '/inc/widgets/js/widget-script.js', array('jquery', 'wp-color-picker', 'jquery-ui-datepicker'), VIRAL_EXPRESS_VER, true);
    wp_localize_script('viral-express-widget-script', 'viral_express_widget_options', array(
        'widgets_block_editor' => $is_widgets_block_editor,
    ));
}

add_action('admin_enqueue_scripts', 'viral_express_admin_scripts', 100);

add_action('elementor/editor/before_enqueue_scripts', 'viral_express_admin_scripts');


/* ADD EDITOR TO CUSTOMIZER */

function viral_express_customizer_editor() {
    ?>
    <div id="ht-wp-editor-widget-container" style="display: none;">
        <a class="ht-wp-editor-widget-close" href="#" title="<?php esc_attr_e('Close', 'viral-express'); ?>"><i class="icofont-close-squared-alt"></i></a>
        <div class="editor">
            <?php
            $settings = array('textarea_rows' => 55, 'editor_height' => 260);
            wp_editor('', 'wpeditorwidget', $settings);
            ?>
            <p><a href="#" class="ht-wp-editor-widget-update-close button button-primary"><?php _e('Save and Close', 'viral-express'); ?></a></p>
        </div>
    </div>
...
add_action('widgets_admin_page', 'viral_express_customizer_editor', 100);
add_action('customize_controls_print_footer_scripts', 'viral_express_customizer_editor');
add_action('elementor/editor/before_enqueue_scripts', 'viral_express_customizer_editor');

//SiteOrigin Builder
if (function_exists('siteorigin_panels_render')) {
    add_action('admin_print_scripts-post.php', 'viral_express_customizer_editor', 100);
    add_action('admin_print_scripts-post-new.php', 'viral_express_customizer_editor', 100);
}

//Beaver Builder
if (class_exists('FLBuilder')) {
    if (isset($_GET['fl_builder'])) {
        add_action('viral_express_after_footer', 'viral_express_customizer_editor', 100);
    }
}
